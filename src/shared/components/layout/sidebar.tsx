...
import {
  ...
  Map, Layers, Award, Settings, ChevronLeft, ChevronRight,
  X, Bell, Share2,
} from 'lucide-react';
...
export function Sidebar() {
  ...
  const { sidebarCollapsed, toggleSidebar, mobileOpen, closeMobileSidebar } = useUIStore();
  ...
  const navItems = [
    ...
    { icon: Bell,            label: t('nav.notifications', 'Notifications'), path: ROUTES.NOTIFICATIONS },
    { icon: Share2,          label: t('nav.referrals', 'Referral Links'),    path: ROUTES.REFERRAL_LINKS },
  ];
  ...
  const NavLink = ({ item }: { item: NavItem }) => {
    ...
    return (
      ...
      >
        ...
        <span className={cn('truncate text-sm font-medium', sidebarCollapsed && 'lg:hidden')}>
          {item.label}
        </span>
      </Link>
    );
      ...
      <div className='flex h-full flex-col'>
        ...
        <div className='flex h-16 items-center justify-between gap-2 border-b px-4'>
          ...
          <Button
            ...
            className={cn('hidden lg:flex flex-shrink-0', sidebarCollapsed && 'mx-auto')}
