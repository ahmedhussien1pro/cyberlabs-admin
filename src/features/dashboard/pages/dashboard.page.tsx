  ...
  return (
    <div className='space-y-6'>
      ...
      {showError && (
        <DashboardErrorAlert
          errorType={errorType}
          warningText={t('warnings.analytics')}
        />
      )}
      ...
      <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-4'>
        {usersLoading ? (
          ...
          <StatsCard
            ...
            value={us?.total ?? ov?.users ?? 0}
            subtitle={`+${us?.newThisMonth ?? 0} this month`}
            change={calcUserChangePercent(
              us?.total ?? 0,
              us?.newThisMonth ?? 0,
            )}
            icon={Users}
        ...
        {coursesLoading ? (
          ...
          <StatsCard
            ...
            value={cs?.total ?? ov?.courses ?? 0}
            subtitle={`${cs?.published ?? 0} published`}
            icon={BookOpen}
            accentColor='green'
          />
        )}
        {labsLoading ? (
          ...
          <StatsCard
            ...
            value={ls?.total ?? ov?.labs ?? 0}
            subtitle={`${ls?.published ?? 0} published`}
            icon={FlaskConical}
            accentColor='purple'
          />
        )}
        {overviewLoading ? (
          ...
          <StatsCard
            ...
            value={ov?.enrollments ?? 0}
            subtitle={`${ov?.labCompletions ?? 0} lab completions`}
            icon={GraduationCap}
            accentColor='orange'
          />
        )}
      ...
      <div className='grid gap-4 md:grid-cols-3'>
        {us && (
          <BreakdownCard
            title={t('breakdown.users') || 'Users Breakdown'}
            icon={Users}
            rows={[
              { label: 'Total', value: us.total },
              {
                label: 'New This Month',
                value: us.newThisMonth,
                badge: 'secondary',
              },
              { label: 'Suspended', value: us.suspended, badge: 'destructive' },
            ]}
            section={{
              heading: 'By Role',
              rows: Object.entries(us.byRole ?? {}) as [string, number][],
            }}
          />
        )}
        {cs && (
          <BreakdownCard
            title={t('breakdown.courses') || 'Courses Breakdown'}
            icon={BookOpen}
            rows={[
              { label: 'Total', value: cs.total },
              { label: 'Published', value: cs.published, badge: 'green' },
              { label: 'Unpublished', value: cs.unpublished, badge: 'outline' },
              { label: 'Featured', value: cs.featured, badge: 'secondary' },
            ]}
            section={{
              heading: 'By State',
              rows: Object.entries(cs.byState ?? {}) as [string, number][],
            }}
          />
        )}
        {ls && (
          <BreakdownCard
            title={t('breakdown.labs') || 'Labs Breakdown'}
            icon={FlaskConical}
            rows={[
              { label: 'Total', value: ls.total },
              { label: 'Published', value: ls.published, badge: 'green' },
              { label: 'Unpublished', value: ls.unpublished, badge: 'outline' },
              {
                label: 'Completions',
                value: ls.totalCompletions,
                badge: 'secondary',
              },
            ]}
            section={{
              heading: 'By Difficulty',
              rows: Object.entries(ls.byDifficulty ?? {}) as [string, number][],
            }}
          />
        )}
      </div>
