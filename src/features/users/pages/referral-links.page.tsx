...
function CreateLinkDialog({
  ...
  const handleCreate = async () => {
    if (!label.trim()) {
      toast.error('أضف تسمية للرابط أولاً');
      return;
    }
    setLoading(true);
    const slug =
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className='max-w-md'>
        <DialogHeader>
          ...
          <DialogDescription>
            أنشئ رابطاً قابلاً للتتبع لمشاركته على السوشيال ميديا
          </DialogDescription>
        </DialogHeader>
        ...
        <div className='space-y-4'>
          <div>
            ...
            <Input
              placeholder='مثال: حملة فيسبوك يناير'
              value={label}
                ...
                return (
                  <button
                    ...
                    <Icon
                      className={cn(
                        'h-4 w-4',
                        source === key ? 'text-blue-400' : cfg.color,
                      )}
                    />
                    {cfg.label}
                  </button>
          ...
          <div>
            <label className='text-xs font-medium text-muted-foreground'>
              تخصيص لمستخدم (اختياري)
            </label>
            <Input
              placeholder='User ID أو اتركه فارغاً لرابط عام'
              value={userId}
...
export default function ReferralLinksPage() {
  ...
  const totalClicks =
    stats?.totalClicks ??
    links.reduce((s: number, l: ReferralLink) => s + l.clicks, 0);
  const totalRegistrations =
    stats?.totalRegistrations ??
    links.reduce((s: number, l: ReferralLink) => s + l.registrations, 0);
            ...
            <div className='space-y-0.5 px-2 pb-3'>
              <button
                ...
                <span className='flex items-center gap-2'>
                  <Globe className='h-3.5 w-3.5' /> جميع المنصات
                </span>
                    ...
                    {srcStat && srcStat.clicks > 0 && (
                      <div className='mt-1 flex gap-3 text-[10px] text-muted-foreground'>
                        ...
                        <span className='text-emerald-400'>
                          {Math.round(
                            (srcStat.registrations / srcStat.clicks) * 100,
                          )}
                          % تحويل
                        </span>
                      </div>
          ...
          <ScrollArea className='flex-1'>
            <div className='space-y-2 p-4'>
              ...
              {filtered.map((link: ReferralLink) => {
                ...
                const conv =
                  link.clicks > 0
                    ? Math.round((link.registrations / link.clicks) * 100)
                    : 0;
                        ...
                        <div>
                          ...
                          <div className='mt-0.5 flex items-center gap-2 flex-wrap'>
                            ...
                            {link.targetUserName && (
                              <Badge
                                variant='outline'
                                className='text-[10px] h-4 border-violet-500/30 text-violet-400'>
                    ...
                    <div className='mt-3 flex items-center gap-2'>
                      ...
                      {shareUrl && (
                        <Button
                          ...
                          onClick={() =>
                            window.open(
                              shareUrl(link.url, 'انضم إلى CyberLabs!'),
                              '_blank',
                            )
                          }>
      ...
      <AlertDialog
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              سيتم حذف <strong>{deleteTarget?.label}</strong> بشكل نهائي ولا
              يمكن استعادته.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
