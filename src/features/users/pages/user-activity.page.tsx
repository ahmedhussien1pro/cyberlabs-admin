...
import {
  ArrowLeft, BookOpen, FlaskConical, Trophy, Zap, LogIn,
...
type ActivityType =
  ...
  | 'LAB_STARTED' | 'LAB_COMPLETED'
  | 'BADGE_EARNED' | 'XP_GAINED'
...
const ACTIVITY_CONFIG: Record<ActivityType, { icon: React.ElementType; color: string; bg: string }> = {
  ...
  COURSE_ENROLLED:  { icon: BookOpen,      color: 'text-blue-400',    bg: 'bg-blue-500/10' },
  COURSE_COMPLETED: { icon: CheckCircle2,  color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
  LAB_STARTED:      { icon: FlaskConical,  color: 'text-violet-400',  bg: 'bg-violet-500/10' },
  LAB_COMPLETED:    { icon: CheckCircle2,  color: 'text-violet-400',  bg: 'bg-violet-500/10' },
  ...
  ROLE_CHANGED:     { icon: Shield,        color: 'text-orange-400',  bg: 'bg-orange-500/10' },
  SUSPENDED:        { icon: XCircle,       color: 'text-destructive', bg: 'bg-destructive/10' },
  UNSUSPENDED:      { icon: CheckCircle2,  color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
};
...
export default function UserActivityPage() {
  ...
  const { data: activityData, isLoading, error } = useQuery({
    ...
    queryFn:  async () => {
      try {
        return await usersService.getActivity(id!, { type: filter === 'ALL' ? undefined : filter, limit: 100 });
      } catch {
  ...
  const initials = user
    ? (user.name ?? user.email).split(' ').slice(0, 2).map((w: string) => w[0]?.toUpperCase()).join('')
    : '?';
    ...
    <div className='flex h-[calc(100vh-4rem)] flex-col overflow-hidden'>
      ...
      <div className='flex items-center justify-between gap-4 border-b border-border/60 bg-background px-5 py-3 flex-wrap'>
        <div className='flex items-center gap-3'>
          <Button variant='ghost' size='icon' onClick={() => navigate(ROUTES.USER_DETAIL(id!))}>
        ...
        <div className='flex items-center gap-4 text-xs text-muted-foreground'>
          <span className='flex items-center gap-1'><Activity className='h-3.5 w-3.5' />{activities.length} {t('header.events')}</span>
          {user?.points && (
            <span className='flex items-center gap-1 text-yellow-400'><Star className='h-3.5 w-3.5' />{t('header.level')} {user.points.level}</span>
          )}
        ...
        <div className='p-5 space-y-6'>
          ...
          {!isLoading && activities.length === 0 && (
            <div className='flex flex-col items-center justify-center gap-3 py-20'>
              ...
              <p className='text-xs text-muted-foreground/60'>{t('empty.subtitle')}</p>
            </div>
          )}
            ...
            <div key={day}>
              ...
              <div className='flex items-center gap-3 mb-3'>
                ...
                <span className='flex items-center gap-1.5 rounded-full border border-border/50 bg-muted/40 px-3 py-1 text-[11px] font-medium text-muted-foreground'>
                  ...
                  {format(new Date(day), 'EEEE, d MMM yyyy', { locale: dateLocale })}
                </span>
                <div className='h-px flex-1 bg-border/50' />
              </div>
              ...
              <div className='space-y-2'>
                {grouped[day].map((item) => {
                  const cfg = ACTIVITY_CONFIG[item.type] ?? ACTIVITY_CONFIG.XP_GAINED;
                  const Icon = cfg.icon;
                  return (
                    <div key={item.id}
                      ...
                      <div className='min-w-0 flex-1'>
                        <div className='flex items-center justify-between gap-2 flex-wrap'>
                          <p className='text-sm font-medium'>
                            {t(`events.${item.type.toLowerCase()}`, item.type.replace(/_/g, ' '))}
                          </p>
                        ...
                        {item.metadata?.courseTitle && (
                          <Badge variant='outline' className='mt-1 text-[10px] border-blue-500/30 text-blue-400 bg-blue-500/5'>
                            <BookOpen className='mr-1 h-2.5 w-2.5' /> {item.metadata.courseTitle}
                          </Badge>
                        )}
                        {item.metadata?.labTitle && (
                          <Badge variant='outline' className='mt-1 text-[10px] border-violet-500/30 text-violet-400 bg-violet-500/5'>
                            <FlaskConical className='mr-1 h-2.5 w-2.5' /> {item.metadata.labTitle}
                          </Badge>
                        )}
                        {item.metadata?.badgeName && (
                          <Badge variant='outline' className='mt-1 text-[10px] border-yellow-500/30 text-yellow-400 bg-yellow-500/5'>
                            <Trophy className='mr-1 h-2.5 w-2.5' /> {item.metadata.badgeName}
                          </Badge>
                        )}
                      </div>
                      {item.xpGained && item.xpGained > 0 && (
                        <Badge className='shrink-0 bg-amber-500/15 text-amber-400 border-amber-500/30 text-[10px]'>
                          <Zap className='mr-1 h-2.5 w-2.5' />+{item.xpGained} XP
                        </Badge>
                      )}
                    </div>
                  );
