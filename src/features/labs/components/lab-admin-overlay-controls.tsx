...
export function LabAdminOverlayControls({ lab }: LabAdminOverlayControlsProps) {
  ...
  const publishMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(lab.isPublished ? `"${lab.title}" unpublished` : `"${lab.title}" published`);
      invalidate();
    },
  ...
  const duplicateMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(`"${lab.title}" duplicated`);
      invalidate();
    },
    onError: () => toast.error('Failed to duplicate lab'),
  });

  const deleteMutation = useMutation({
    mutationFn: () => labsService.delete(lab.id),
    onSuccess: () => {
      toast.success(`"${lab.title}" deleted`);
      invalidate();
    },
    onError: (err: any) => {
    ...
    <>
      ...
      >
        ...
        <div className='flex items-center justify-between'>
          ...
          >
            {lab.isPublished ? <><EyeOff className='h-3 w-3' /> Unpublish</> : <><Globe className='h-3 w-3' /> Publish</>}
          </Button>
          <a
            href={platformUrl}
            target='_blank'
        ...
        <div className='flex items-center gap-1.5'>
          <Button
            ...
            onClick={(e) => { e.preventDefault(); navigate(ROUTES.LAB_EDIT(lab.id)); }}
          >
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete "{lab.title}"?</AlertDialogTitle>
            <AlertDialogDescription>
              This will permanently delete the lab and all its data. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
