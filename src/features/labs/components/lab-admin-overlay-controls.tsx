...
interface Props { lab: LabListItem; }

export function LabAdminOverlayControls({ lab }: Props) {
  ...
  const publishMutation = useMutation({
    mutationFn: () => lab.isPublished ? labsService.unpublish(lab.id) : labsService.publish(lab.id),
    onSuccess: () => {
      toast.success(lab.isPublished
        ? t('unpublishedMsg', { title: lab.title })
        : t('publishedMsg', { title: lab.title }));
      invalidate();
    },
  ...
  const duplicateMutation = useMutation({
    ...
    onSuccess: () => { toast.success(t('duplicatedMsg', { title: lab.title })); invalidate(); },
    onError: () => toast.error(t('failedDuplicate')),
  });

  const deleteMutation = useMutation({
    mutationFn: () => labsService.delete(lab.id),
    onSuccess: () => { toast.success(t('deletedMsg', { title: lab.title })); invalidate(); },
    onError: (err: any) => {
      toast.error(err?.response?.data?.message ?? err?.message ?? t('failedStatus'));
    },
  });
