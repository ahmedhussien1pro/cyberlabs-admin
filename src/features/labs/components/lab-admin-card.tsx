...
import { FlaskConical, Terminal, Users, BarChart3, Target, Clock, BookOpen } from 'lucide-react';
...
const EXEC_ICON: Record<string, React.ElementType> = {
  BROWSER: BookOpen, DOCKER: Terminal, VM: Terminal,
  FRONTEND: BookOpen, SHARED_BACKEND: Terminal, STATIC: BookOpen,
};
...
function LabThumbnail({ lab, className }: { lab: LabListItem; className?: string }) {
  ...
  if (img) return <img src={img} alt={lab.title} loading="lazy" className={cn('w-full h-full object-cover', className)} />;
  return (
    <div className={cn('w-full h-full flex items-center justify-center bg-gradient-to-br border', DIFF_BG[diff] ?? 'from-zinc-900 to-zinc-800 border-zinc-700')}>
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        DIFF_RING[diff], 'hover:shadow-xl hover:-translate-y-0.5',
      )}
        ...
        {lab.executionMode && (
          <div className="absolute top-2 end-2 z-20 pointer-events-none">
            <span className="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-bold bg-black/40 text-white/80 border border-white/10">
              ...
              {t(lab.executionMode as any, { defaultValue: lab.executionMode })}
            </span>
          </div>
        )}
      </div>
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          {lab.difficulty && (
            <Badge variant="outline" className={cn('gap-1 text-[10px] font-semibold', DIFF_BADGE[lab.difficulty])}>
              ...
              {t(lab.difficulty as any, { defaultValue: lab.difficulty })}
            </Badge>
          )}
          {lab.category && (
            <Badge variant="outline" className="gap-1 text-[10px] text-muted-foreground border-border/40">
              <Target className="h-3 w-3" />
              {t(lab.category as any, { defaultValue: lab.category.replace(/_/g, ' ') })}
            </Badge>
          )}
          {(lab as any).duration && (
            <Badge variant="outline" className="gap-1 text-[10px] text-muted-foreground border-border/40">
