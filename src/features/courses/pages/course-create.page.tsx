...
import CourseInfoForm, {
  DEFAULT_INFO,
} from '../components/content-editor/CourseInfoForm';
...
function normalizeImportedTopics(raw: any[]): {
  topics: Topic[];
  warnings: string[];
} {
      ...
      const elements = Array.isArray(t.elements)
        ? t.elements
            .map((el: any, ei: number) => {
              if (!el?.type) {
                warnings.push(
                  `Topic ${ti + 1}, Element ${ei + 1}: missing type`,
                );
                return null;
              }
...
function parseJsonFile(content: string): ImportPreview {
  ...
  if (Array.isArray(parsed)) {
    ...
    if (Array.isArray(parsed.topics)) {
      ...
      warnings.push(
        'No "topics" or "curriculum" array found — treating entire file as single topic',
      );
      rawTopics = [parsed];
    }
...
function JsonImportModal({
  ...
  const handleFile = (file: File) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (p.topicCount === 0) {
          setError('No valid topics found in this file');
          return;
        }
        setPreview(p);
      } catch (err: any) {
  ...
  const handleConfirm = () => {
    ...
    onImport({
      ...
      topics: preview.topics.map((t) => ({ ...t, _importMode: mode }) as any),
    });
    onClose();
  };
      ...
      <div className='w-full max-w-lg rounded-xl border bg-card shadow-2xl'>
        ...
        <div className='flex items-center justify-end gap-2 px-5 py-3 border-t'>
          ...
          <Button
            ...
            Import{' '}
            {preview
              ? `${preview.topicCount} topic${preview.topicCount !== 1 ? 's' : ''}`
              : ''}
          </Button>
        </div>
      </div>
...
function InstructorPicker({
  ...
  const { data: adminData } = useQuery({
    ...
    queryFn: () =>
      usersService.getAll({ page: 1, limit: 100, role: 'ADMIN' as any }),
  });
  const { data: instructorData } = useQuery({
    queryKey: ['users', 'instructors', 'INSTRUCTOR'],
    queryFn: () =>
      usersService.getAll({ page: 1, limit: 100, role: 'INSTRUCTOR' as any }),
  });
...
export default function CourseCreatePage() {
  ...
  const [createdCourseSlug, setCreatedCourseSlug] = useState<string | null>(
    null,
  );
  ...
  const handleImageUpload = (key: string, file: File) => {
    const reader = new FileReader();
    reader.onloadend = () =>
      setImageMap((m) => ({ ...m, [key]: reader.result as string }));
  ...
  const handleImport = (preview: ImportPreview) => {
    ...
    const cleanTopics = preview.topics.map(
      ({ _importMode, ...t }: any) => t as Topic,
    );
  ...
  const { mutate: createCourse, isPending: isCreating } = useMutation({
    ...
    onSuccess: (course) => {
      ...
      toast.success(`Course "${course.title}" created!`, {
        description: 'Now add topics and content.',
      });
      setCourseInfo((c) => ({
        ...c,
        title: { en: meta.title, ar: meta.ar_title },
      }));
    },
    onError: (err: any) => {
      const msg =
  ...
  const { mutate: saveCurriculum, isPending: isSaving } = useMutation({
    mutationFn: async () => {
      ...
      return coursesApi.saveCurriculum(
        createdCourseId,
        topics.map((t) => ({
          ...t,
  ...
  return (
    <div className='flex h-[calc(100vh-4rem)] overflow-hidden'>
      ...
      {/* Sidebar */}
      <aside
        className={`flex-shrink-0 flex flex-col border-r bg-card transition-all duration-200 ${sidebarCollapsed ? 'w-12' : 'w-64'}`}>
          ...
          <>
            <nav className='p-2 space-y-1'>
              <button
                ...
                {!createdCourseId && (
                  <span className='ml-auto text-xs bg-yellow-500/20 text-yellow-400 px-1.5 rounded'>
                    Step 1
                  </span>
                )}
              </button>
            ...
            <div className='p-3 border-t space-y-2'>
              ...
              {createdCourseSlug && (
                <Button
                  ...
                  onClick={() =>
                    navigate(`/courses/${createdCourseSlug}/edit`)
                  }>
                  Open in Editor
                </Button>
              )}
                    ...
                    <form onSubmit={handleCreateSubmit} className='space-y-4'>
                      <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
                        ...
                        <div>
                          ...
                          <input
                            ...
                            value={meta.ar_title}
                            onChange={(e) =>
                              setMeta_('ar_title', e.target.value)
                            }
                          />
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className='flex items-center gap-2'>
                          ...
                          <input
                            ...
                            value={meta.slug}
                            onChange={(e) =>
                              setMeta_('slug', slugify(e.target.value))
                            }
                            required
                          />
                        </div>
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            setMeta_('description', e.target.value)
                          }
                        />
                      </div>
                      ...
                      <div className='grid grid-cols-2 sm:grid-cols-3 gap-3'>
                        ...
                        <div className='col-span-2'>
                          ...
                          <select
                            ...
                            value={meta.category}
                            onChange={(e) =>
                              setMeta_('category', e.target.value)
                            }>
                            {CATEGORIES.map((c) => (
