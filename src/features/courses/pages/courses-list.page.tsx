  ...
  return (
    <div className='mt-6 flex flex-col items-center gap-3 border-t pt-5 sm:flex-row sm:justify-between'>
      <p className='text-xs text-muted-foreground order-2 sm:order-1'>
        ...
        {t('table.of')}{' '}<span className='font-semibold text-foreground'>{total}</span>{' '}{t('table.results', 'results')}
      </p>
      <div className='flex items-center gap-1 order-1 sm:order-2'>
        <Button variant='outline' size='sm' className='hidden sm:flex h-8 w-8 p-0' onClick={() => onPage(1)} disabled={page === 1}>
            ...
            p === '...' ? (
              ...
              <button key={p} onClick={() => onPage(p as number)}
                className={cn(
                  ...
                  p === page ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-background text-foreground hover:bg-muted',
                )}>{p}</button>
            ),
          )}
        </div>
        ...
        <Button variant='outline' size='sm' className='hidden sm:flex h-8 w-8 p-0' onClick={() => onPage(totalPages)} disabled={page === totalPages}>
...
export default function CoursesListPage() {
  ...
  const goPage = useCallback((p: number) => { setPage(p); window.scrollTo({ top: 0, behavior: 'smooth' }); }, []);
    ...
    <div className='space-y-6'>
      ...
      <div className='grid grid-cols-2 gap-3 lg:grid-cols-4'>
        {statsLoading
          ...
          : STAT_CARDS.map(({ key, labelKey, icon: Icon, color, bg }) => {
              ...
              return (
                <Card key={key}
                  className={cn('flex items-center gap-4 p-4 cursor-pointer hover:bg-muted/30 transition-all', isActive && 'ring-2 ring-primary/40')}
      ...
      <div className='flex items-center gap-2 flex-wrap'>
        <div className='relative flex-1 min-w-[180px]'>
          ...
          <Input placeholder={t('filters.searchPlaceholder')} value={search} onChange={handleSearchChange}
      ...
      {isLoading ? (
        ...
        <>
          ...
          {data?.meta && (
            <Pagination page={page} totalPages={data.meta.totalPages} total={data.meta.total} limit={LIMIT} onPage={goPage} />
          )}
        </>
      ) : (
        <CoursesTable data={data?.data ?? []} meta={data?.meta} page={page} onPageChange={goPage} onRefetch={refetch} />
      )}

      {/* Controlled dialog — opened by header button or empty-state button */}
