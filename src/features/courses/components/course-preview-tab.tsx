...
// ─── MatrixRain (lightweight, no canvas) ─────────────────────────────
function MatrixRainCanvas({ color = '#3b82f6' }: { color?: string }) {
  return (
    <div
...
function PreviewHero({
  ...
  return (
    <section className='relative overflow-hidden border-b border-white/8 bg-zinc-950'>
      ...
      <MatrixRainCanvas color={MATRIX_COLOR[col]} />
      <div
        aria-hidden='true'
        className={cn(
          ...
          <div className='flex flex-wrap items-center gap-x-5 gap-y-1.5'>
            ...
            {(c.averageRating ?? 0) > 0 && (
              <div className='flex items-center gap-1.5 text-xs'>
                ...
                <span className='font-bold text-white'>{c.averageRating}</span>
                <span className='text-white/45'>({c.reviewCount})</span>
              </div>
            )}
          </div>

          <div className='flex items-center gap-2'>
            <button className='flex items-center gap-1.5 rounded-lg border border-white/15 px-3 py-1.5 text-xs font-medium text-white/50'>Save</button>
            {comingSoon ? (
...
export function CoursePlatformPreviewTab({ course }: Props) {
  ...
  const handleOpenPreviewWindow = () => {
    ...
    const stripeColor = ({ emerald:'#10b981',blue:'#3b82f6',violet:'#8b5cf6',rose:'#f43f5e',orange:'#f97316',cyan:'#06b6d4' } as any)[col] ?? '#3b82f6';
    const imgSrc = course.image ?? course.thumbnail ?? '';
    const fallbackBgHex = ({ emerald:'#0a2e1e',blue:'#0a1628',violet:'#1a0a2e',orange:'#2e1a0a',rose:'#2e0a14',cyan:'#0a2228' } as any)[col] ?? '#0a1628';
    const accentColor = ({ emerald:'#10b981',blue:'#3b82f6',violet:'#8b5cf6',rose:'#f43f5e',orange:'#f97316',cyan:'#06b6d4' } as any)[col] ?? '#3b82f6';

    const sectionsHTML = sections.map((s, idx) => {
      const num = String(idx + 1).padStart(2, '0');
      const elCount = s.elements?.length ?? 0;
      return `<li style="position:relative;display:flex;gap:16px;margin-bottom:8px;"><div style="display:flex;flex-direction:column;align-items:center;"><div style="width:50px;height:50px;border-radius:50%;border:2px solid ${accentColor}40;background:${accentColor}15;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:14px;color:${accentColor};flex-shrink:0;">${num}</div>${idx < sections.length - 1 ? `<div style="width:1px;flex:1;min-height:16px;background:${accentColor}20;margin-top:4px;"></div>` : ''}</div><div style="flex:1;min-width:0;margin-bottom:8px;border-radius:12px;border:1px solid ${accentColor}30;background:#0d0d0d;overflow:hidden;"><div style="display:flex;align-items:center;gap:12px;padding:14px 16px;"><div style="width:32px;height:32px;border-radius:8px;border:1px solid ${accentColor}30;background:${accentColor}10;display:flex;align-items:center;justify-content:center;flex-shrink:0;"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="${accentColor}" stroke-width="2"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg></div><div style="flex:1;min-width:0;"><div style="margin-bottom:2px;"><span style="font-size:9px;font-weight:700;border:1px solid ${accentColor}25;background:${accentColor}10;color:${accentColor};border-radius:99px;padding:1px 6px;text-transform:uppercase;letter-spacing:0.05em;">TOPIC ${num}</span></div><p style="font-size:14px;font-weight:600;color:#f4f4f5;margin:0;">${s.title}</p>${s.ar_title ? `<p style="font-size:11px;color:#71717a;margin:2px 0 0;direction:rtl;">${s.ar_title}</p>` : ''}</div><span style="font-size:11px;border:1px solid #3f3f46;border-radius:6px;padding:2px 8px;color:#71717a;flex-shrink:0;">${elCount} element${elCount !== 1 ? 's' : ''}</span></div></div></li>`;
    }).join('');

    const accessColor = ({ FREE:'#10b981',PRO:'#3b82f6',PREMIUM:'#8b5cf6' } as any)[course.access] ?? '#10b981';
    const accessIcon = course.access === 'FREE' ? '🔓' : '👑';

    win.document.write(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><title>Preview: ${course.title}</title><style>*,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:#09090b;color:#f4f4f5;min-height:100vh;}.container{max-width:1100px;margin:0 auto;padding:0 16px;}.badge{display:inline-flex;align-items:center;gap:4px;border-radius:99px;padding:2px 8px;font-size:11px;font-weight:700;border:1px solid;}.stat{display:flex;align-items:center;gap:6px;font-size:12px;}.stat-val{font-weight:700;color:#f4f4f5;}.stat-lbl{color:rgba(255,255,255,0.45);}.btn{display:inline-flex;align-items:center;gap:6px;border-radius:8px;padding:6px 20px;font-size:12px;font-weight:600;cursor:pointer;border:none;}</style></head><body><div style="background:rgba(245,158,11,0.1);border-bottom:1px solid rgba(245,158,11,0.3);padding:8px 16px;display:flex;align-items:center;gap:8px;font-size:12px;color:#fbbf24;position:sticky;top:0;z-index:100;">Admin Preview — exact platform view<span style="margin-left:auto;background:rgba(245,158,11,0.15);border:1px solid rgba(245,158,11,0.4);border-radius:6px;padding:1px 8px;font-size:11px;">${course.state}</span></div><section style="position:relative;overflow:hidden;border-bottom:1px solid rgba(255,255,255,0.08);background:#09090b;"><div style="position:absolute;top:0;left:0;right:0;height:3px;background:${stripeColor};"></div><div style="position:absolute;inset:0;background:repeating-linear-gradient(0deg,transparent,transparent 2px,${matrixColor}15 2px,${matrixColor}15 4px);opacity:0.07;pointer-events:none;"></div><div class="container" style="position:relative;z-index:2;"><div style="padding:24px 0 0;"><nav style="margin-bottom:16px;display:flex;align-items:center;gap:4px;font-size:11px;color:rgba(255,255,255,0.35);"><span>Courses</span><span>›</span><span style="color:rgba(255,255,255,0.65);">${course.title}</span></nav><div style="display:flex;align-items:flex-start;gap:16px;"><div style="width:56px;height:56px;border-radius:16px;overflow:hidden;border:1px solid rgba(255,255,255,0.1);flex-shrink:0;">${imgSrc ? `<img src="${imgSrc}" style="width:100%;height:100%;object-fit:cover;" alt="" />` : `<div style="width:100%;height:100%;background:${fallbackBgHex};display:flex;align-items:center;justify-content:center;"><span style="font-size:9px;font-weight:900;color:${accentColor};">${course.title}</span></div>`}</div><div style="flex:1;min-width:0;"><div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:8px;"><span class="badge" style="border-color:${accessColor}40;color:${accessColor};background:${accessColor}10;">${accessIcon} ${course.access}</span><span class="badge" style="border-color:rgba(255,255,255,0.2);color:rgba(255,255,255,0.65);">${course.difficulty}</span></div><h1 style="font-size:clamp(20px,3vw,30px);font-weight:900;color:#fff;line-height:1.2;margin-bottom:4px;">${course.title}</h1>${course.description ? `<p style="font-size:13px;line-height:1.65;color:rgba(255,255,255,0.6);max-width:600px;margin-top:8px;">${course.description}</p>` : ''}</div></div></div><div style="display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:12px;border-top:1px solid rgba(255,255,255,0.1);padding:12px 0;margin-top:16px;"><div style="display:flex;flex-wrap:wrap;gap:20px;"><div class="stat"><span style="color:${accentColor};">&#128218;</span><span class="stat-val">${topicCount}</span><span class="stat-lbl">Topics</span></div>${course.estimatedHours != null ? `<div class="stat"><span style="color:${accentColor};">&#9201;</span><span class="stat-val">${course.estimatedHours}h</span><span class="stat-lbl">est.</span></div>` : ''}</div><div style="display:flex;align-items:center;gap:8px;"><button class="btn" style="background:${accentColor};color:#fff;">${course.access === 'FREE' ? '⚡ Start Free →' : `👑 Upgrade to ${course.access} →`}</button></div></div></div></section><div class="container" style="padding-top:40px;padding-bottom:80px;"><div style="margin-bottom:32px;"><h2 style="font-size:22px;font-weight:700;">Course Curriculum</h2><p style="font-size:13px;color:#71717a;margin-top:4px;">${topicCount} Topics</p></div>${sections.length === 0 ? '<p style="font-size:13px;color:#71717a;font-style:italic;">Curriculum not available yet.</p>' : `<div style="position:relative;"><ol style="list-style:none;">${sectionsHTML}</ol></div>`}</div></body></html>`);
    win.document.close();
  };
  ...
  return (
    <div className='space-y-8'>
      ...
      <div className='flex justify-center'>
        <Button size='lg' className='gap-2.5 min-w-[220px] font-semibold' onClick={handleOpenPreviewWindow}>
      ...
      <div>
        ...
        {isLoading ? (
          ...
          <PlatformCurriculum sections={sections} estimatedHours={course.estimatedHours ?? null} />
        ) : (
          <div className='rounded-xl border border-border/40 bg-muted/20 p-6 text-center'>
            ...
            <p className='text-xs text-muted-foreground/60 mt-1'>Add topics in the Content Writer tab to see them here.</p>
          </div>
        )}
      </div>
