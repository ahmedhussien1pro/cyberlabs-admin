// hero-info-tab.tsx
// ✅ Imports from unified types barrel + shared/ + previews/
...
import type { AdminCourse, HeroFormState } from '../../types';
...
import { LiveHeroPreview } from '../previews/LiveHeroPreview';

interface Props { course: AdminCourse; onSaved: () => void; }

const REQUIRED: Array<{ key: keyof HeroFormState; label: string; labelAr: string }> = [
  { key: 'title',       label: 'Title (EN)',       labelAr: 'العنوان (EN)' },
  { key: 'description', label: 'Description (EN)', labelAr: 'الوصف (EN)'  },
];

const STATE_OPTIONS = ['DRAFT', 'PUBLISHED', 'COMING_SOON'] as const;
...
export function HeroInfoTab({ course, onSaved }: Props) {
  ...
  const set = (k: keyof HeroFormState, v: unknown) => setForm((f) => ({ ...f, [k]: v }));
    ...
    if (missing.length) {
      toast.error(
        <div className='flex items-start gap-2'>
          ...
          <div>
            <p className='font-semibold'>{lbl('Required fields missing', 'حقول مطلوبة')}</p>
  ...
  const mut = useMutation({
    mutationFn: () => adminCoursesApi.update(course.id, {
      ...
      state:              form.state              as AdminCourse['state'],
      isNew:              form.isNew,
    ...
    onSuccess: () => { toast.success(lbl('Hero info saved ✔', 'تم حفظ بيانات الهيرو ✔')); onSaved(); },
    onError:   () => toast.error(lbl('Failed to save', 'فشل الحفظ')),
  });
    ...
    <div className='space-y-6' dir={isAr ? 'rtl' : 'ltr'}>
          ...
          <div className='grid grid-cols-2 gap-4 sm:grid-cols-3'>
            <div className='space-y-1.5'>
              ...
              <Select value={form.color} onValueChange={(v) => set('color', v.toLowerCase())}>
                <SelectTrigger><div className='flex items-center gap-2'><ColorDot color={form.color} /><SelectValue /></div></SelectTrigger>
                <SelectContent>
                  {COURSE_COLORS.map((c) => (
          ...
          <div className='space-y-1.5'>
            ...
            <div className='flex items-center gap-2'>
              <Input dir='ltr' value={form.instructorId} onChange={(e) => set('instructorId', e.target.value.trim())}
                ...
                className={cn('font-mono text-xs', instructorIdInvalid && 'border-amber-500/60')} />
              {form.instructorId && (
            ...
            {instructorIdInvalid && <p className='text-[11px] text-amber-500 flex items-center gap-1'><AlertTriangle className='h-3 w-3' /> {lbl('Invalid UUID format', 'صيغة UUID غير صحيحة')}</p>}
            {form.instructorId && UUID_RE.test(form.instructorId) && <p className='text-[11px] text-emerald-500/80'>✔ {lbl('Valid UUID', 'UUID صحيح')}</p>}
            {!form.instructorId && <p className='text-[11px] text-muted-foreground/50'>{lbl('Leave empty to unassign', 'اتركه فارغاً لإلغاء التعيين')}</p>}
          </div>

          <div className='flex flex-wrap items-center gap-6 pt-2'>
            <div className='flex items-center gap-2'><Switch checked={form.isNew} onCheckedChange={(v) => set('isNew', v)} id='hero-isNew' /><Label htmlFor='hero-isNew'>{lbl('Is New', 'جديد')}</Label></div>
            <div className='flex items-center gap-2'><Switch checked={form.isFeatured} onCheckedChange={(v) => set('isFeatured', v)} id='hero-featured' /><Label htmlFor='hero-featured'>{lbl('Featured', 'مميز')}</Label></div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className='space-y-4'>
          <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
            ...
            <div className='space-y-1.5'>
              ...
              <Textarea dir='ltr' rows={5} value={form.longDescription} placeholder='Detailed description…' onChange={(e) => set('longDescription', e.target.value)} />
            </div>
            <div className='space-y-1.5'>
      ...
      <Dialog open={previewOpen} onOpenChange={setPreviewOpen}>
        <DialogContent className='max-w-2xl max-h-[90vh] overflow-y-auto'>
          <DialogHeader>
            <DialogTitle className='flex items-center gap-2'><Eye className='h-4 w-4' /> {lbl('Hero Preview (unsaved)', 'معاينة الهيرو (غير محفوظ)')}</DialogTitle>
            <DialogDescription className='text-xs'>{lbl('Live preview — not saved yet.', 'معاينة حية — لم يُحفظ بعد.')}</DialogDescription>
          </DialogHeader>
          <div className='py-2'><LiveHeroPreview form={form} course={course} isAr={isAr} /></div>
          <div className='rounded-lg border border-border/40 bg-muted/20 p-3 space-y-1.5'>
            ...
            {([
              { label: 'Title EN',     old: course.title,                        cur: form.title },
              { label: 'Title AR',     old: course.ar_title,                     cur: form.ar_title },
              { label: 'Color',        old: (course.color ?? '').toLowerCase(),  cur: form.color },
              { label: 'Access',       old: course.access,                       cur: form.access },
              { label: 'Difficulty',   old: course.difficulty,                   cur: form.difficulty },
              { label: 'State',        old: course.state,                        cur: form.state },
              { label: 'Hours',        old: String(course.estimatedHours ?? 0), cur: form.estimatedHours },
              { label: 'Instructor',   old: course.instructorId ?? '',           cur: form.instructorId },
            ...
            {course.title === form.title && (course.color ?? '') === form.color && course.state === form.state && (
