// src/features/courses/components/previews/LiveHeroPreview.tsx
// Live hero preview — mirrors CoursePlatformPreviewTab hero section exactly.
        ...
        <div className='bg-zinc-900/60 px-5 py-3 flex flex-wrap gap-4 border-t border-white/5 mt-4'>
          {form.skills.length > 0 && (
            <div>
              ...
              <div className='flex flex-wrap gap-1'>
                {(isAr && form.ar_skills.length > 0 ? form.ar_skills : form.skills)
                  .slice(0, 4).map((s) => (
          ...
          {form.prerequisites.length > 0 && (
            <div>
              ...
              <div className='flex flex-wrap gap-1'>
                {(isAr && form.ar_prerequisites.length > 0 ? form.ar_prerequisites : form.prerequisites)
                  .slice(0, 3).map((s) => (
                    <span key={s} className='rounded-full border border-white/10 bg-white/5 px-2 py-0.5 text-[10px] text-white/50'>{s}</span>
                  ))}
              </div>
