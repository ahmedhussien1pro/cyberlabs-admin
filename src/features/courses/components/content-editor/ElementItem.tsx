  ...
  const renderFields = () => {
    switch (element.type) {
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
            <div>
              ...
              <RichTextEditor
                ...
                onChange={(v) =>
                  onChange({ ...element, value: { ...element.value, en: v } })
                }
              />
            </div>
            <div>
              ...
              <RichTextEditor
                ...
                onChange={(v) =>
                  onChange({ ...element, value: { ...element.value, ar: v } })
                }
                dir='rtl'
              />
            </div>
        ...
        return (
          <div className='space-y-3'>
            ...
            <div className='flex gap-2'>
              <button
                type='button'
                className={`flex items-center gap-1 px-3 py-1.5 rounded-md text-xs border transition-colors ${element.imageMode !== 'upload' ? 'bg-primary text-primary-foreground border-primary' : ''}`}
                onClick={() =>
            ...
            {(imageMap[element.srcKey ?? ''] || element.imageUrl) && (
              <div className='relative mt-2'>
                <button
                  type='button'
                  className='absolute top-1 right-1 bg-black/50 rounded-full p-1 text-white z-10'
                  onClick={() => onChange({ ...element, imageUrl: '' })}>
                  <X size={12} />
                </button>
                <img
                  src={imageMap[element.srcKey ?? ''] || element.imageUrl}
                  alt='preview'
                  className='rounded-md max-h-48 object-contain w-full border'
                />
              </div>
            )}
          </div>
      ...
      case 'note': {
        ...
        return (
          <div
            className={`space-y-3 p-3 rounded-md border ${NOTE_COLORS[element.noteType ?? 'info']}`}>
            <div className='grid grid-cols-2 gap-3'>
              <div>
                <label className='text-xs text-muted-foreground mb-1 block'>
                  Note Type
                ...
                {(element.items?.en ?? []).map((item: string, i: number) => (
                  <div key={i} className='flex gap-1 mb-1'>
                    <input
                      ...
                      value={item}
                      onChange={(e) => {
                        const ne = [...element.items.en];
                        ne[i] = e.target.value;
                        onChange({
                          ...element,
                          items: { ...element.items, en: ne },
                        });
                      }}
                    />
                    <button
                      ...
                      onClick={() =>
                        onChange({
                          ...element,
                          items: {
                            en: element.items.en.filter(
                              (_: any, idx: number) => idx !== i,
                            ),
                            ar: element.items.ar.filter(
                              (_: any, idx: number) => idx !== i,
                            ),
                          },
                        })
                ...
                {(element.items?.ar ?? []).map((item: string, i: number) => (
                  <div key={i} className='flex gap-1 mb-1'>
                    <input
                      ...
                      value={item}
                      onChange={(e) => {
                        const ne = [...element.items.ar];
                        ne[i] = e.target.value;
                        onChange({
                          ...element,
                          items: { ...element.items, ar: ne },
                        });
                      }}
                    />
                  </div>
                ...
                <div className='grid grid-cols-2 gap-2'>
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      ni[i] = {
                        ...ni[i],
                        subtitle: { ...ni[i].subtitle, en: e.target.value },
                      };
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      ni[i] = {
                        ...ni[i],
                        subtitle: { ...ni[i].subtitle, ar: e.target.value },
                      };
                      onChange({ ...element, items: ni });
                    }}
                  />
                ...
                <div className='grid grid-cols-2 gap-2'>
                  <textarea
                    ...
                    onChange={(e) => {
                      ...
                      ni[i] = {
                        ...ni[i],
                        text: { ...ni[i].text, en: e.target.value },
                      };
                      onChange({ ...element, items: ni });
                    }}
                  />
                  <textarea
                    ...
                    onChange={(e) => {
                      ...
                      ni[i] = {
                        ...ni[i],
                        text: { ...ni[i].text, ar: e.target.value },
                      };
                      onChange({ ...element, items: ni });
                    }}
                  />
        ...
        return (
          <div className='space-y-3'>
            <div className='grid grid-cols-2 gap-3'>
              <input
                ...
                onChange={(e) =>
                  onChange({
                    ...element,
                    title: { en: e.target.value, ar: element.title?.ar ?? '' },
                  })
                }
              />
              <input
                ...
                onChange={(e) =>
                  onChange({
                    ...element,
                    title: { en: element.title?.en ?? '', ar: e.target.value },
                  })
                }
              />
            </div>
            ...
            {(element.rows ?? []).map((row: any, ri: number) => (
              <div
                key={ri}
                className='border rounded-md p-2 space-y-1 bg-muted/10'>
                <div className='flex items-center justify-between'>
                  <span className='text-xs text-muted-foreground'>
                    Row {ri + 1}
                  </span>
                ...
                <div className='flex gap-1 flex-wrap'>
                  {row.en.map((cell: string, ci: number) => (
                    <input
                      key={ci}
                      className='flex-1 min-w-20 rounded border bg-background px-2 py-1 text-xs'
                      placeholder={element.headers?.en[ci] ?? `Col${ci + 1}`}
                      value={cell}
                      onChange={(e) => {
                        const nr = [...element.rows!];
                        nr[ri] = { ...nr[ri], en: [...nr[ri].en] };
                        nr[ri].en[ci] = e.target.value;
                ...
                <div className='flex gap-1 flex-wrap'>
                  {row.ar.map((cell: string, ci: number) => (
                    <input
                      key={ci}
                      className='flex-1 min-w-20 rounded border bg-background px-2 py-1 text-xs'
                      dir='rtl'
                      placeholder={element.headers?.ar[ci] ?? `عمود${ci + 1}`}
                      value={cell}
                      onChange={(e) => {
                        const nr = [...element.rows!];
                        nr[ri] = { ...nr[ri], ar: [...nr[ri].ar] };
                        nr[ri].ar[ci] = e.target.value;
                        onChange({ ...element, rows: nr });
                      }}
                    />
                  ))}
      ...
      default:
        return (
          <p className='text-xs text-muted-foreground'>
            Element type "{element.type}" not supported yet.
          </p>
        );
    }
  };
    ...
    <div className='border rounded-lg bg-card overflow-hidden'>
      <div
        ...
        <div className='flex items-center gap-2'>
          ...
          <span
            className={`text-xs font-semibold px-2 py-0.5 rounded border ${badgeCls}`}>
            {element.type}
          </span>
          <span className='text-xs text-muted-foreground'>
            Element #{index + 1}
