...
import {
  Edit2, Globe, EyeOff, Clock, Trash2, Copy,
...
export function CoursesTable({ data, meta, page, onPageChange, onRefetch }: Props) {
  ...
  const deleteMut = useMutation({
    ...
    onSuccess: () => { toast.success(t('toast.stateUpdated')); invalidate(); onRefetch(); setDeleteTarget(null); },
    onError: (err: any, _v, ctx: any) => {
      if (ctx?.snapshot) ctx.snapshot.forEach(([key, val]: [any, any]) => qc.setQueryData(key, val));
                ...
                <tr key={course.id} className='border-b border-border/50 hover:bg-muted/20 transition-colors'>
                  ...
                  <td className='px-4 py-3'>
                    <div className='flex items-center justify-end gap-1'>
                      <Button variant='ghost' size='sm' aria-label={`${t('overlay.edit')} ${course.title}`}
                        className='h-8 w-8 p-0'
                        onClick={() => navigate(ROUTES.COURSE_EDIT(course.slug))}>
                        <Edit2 className='h-3.5 w-3.5' aria-hidden='true' />
                      </Button>
                      <Button variant='ghost' size='sm' aria-label={`${t('overlay.duplicate')} ${course.title}`}
                        className='h-8 w-8 p-0' disabled={dupMut.isPending}
