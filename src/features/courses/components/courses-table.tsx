...
export function CoursesTable({ data, meta, page, onPageChange, onRefetch }: Props) {
  ...
  const deleteMut = useMutation({
    ...
    onMutate: async (id) => {
      ...
      const snapshot = qc.getQueriesData({ queryKey: ['admin', 'courses', 'list'] });
      qc.setQueriesData({ queryKey: ['admin', 'courses', 'list'] }, (old: any) =>
        old?.data ? { ...old, data: old.data.filter((c: AdminCourse) => c.id !== id) } : old,
      );
      return { snapshot };
    },
    onSuccess: () => {
      toast.success(t('toast.deleted', { title: deleteTarget?.title }));
      invalidate(); onRefetch(); setDeleteTarget(null);
    },
    onError: (err: any, _v: any, ctx: any) => {
      if (ctx?.snapshot) ctx.snapshot.forEach(([key, val]: [any, any]) => qc.setQueryData(key, val));
      toast.error(t('errors.deleteFailed', { message: err?.response?.data?.message ?? err?.message ?? 'Unknown error' }));
    },
  });
