...
import {
  Edit2, Globe, EyeOff, Clock, Trash2, Copy, Eye,
...
const STATE_OPTIONS: { value: CourseState; icon: React.ElementType; color: string }[] = [
...
export function CoursesTable({ data, meta, page, onPageChange, onRefetch }: Props) {
    ...
    onMutate: async ({ id, state }) => {
      ...
      qc.setQueriesData(
        ...
        (old: any) => {
          ...
          return { ...old, data: old.data.map((c: AdminCourse) => c.id === id ? { ...c, state } : c) };
        },
      );
      return { snapshot };
    },
    onSuccess: () => { toast.success(t('toast.stateUpdated')); invalidate(); onRefetch(); },
    onError: (_e: any, _v: any, ctx: any) => {
      if (ctx?.snapshot) ctx.snapshot.forEach(([key, val]: [any, any]) => qc.setQueryData(key, val));
  ...
  const deleteMut = useMutation({
    ...
    onSuccess: () => { toast.success(t('toast.deleted', { title: deleteTarget?.title })); invalidate(); onRefetch(); setDeleteTarget(null); },
          ...
          <table className='w-full text-sm' role='table'>
            <thead>
              <tr className='border-b bg-muted/30'>
                <th className='px-4 py-3 text-start font-semibold text-muted-foreground' scope='col'>{t('table.course')}</th>
                <th className='px-4 py-3 text-start font-semibold text-muted-foreground' scope='col'>{t('table.state')}</th>
                <th className='px-4 py-3 text-start font-semibold text-muted-foreground' scope='col'>{t('table.difficulty')}</th>
                <th className='px-4 py-3 text-start font-semibold text-muted-foreground' scope='col'>{t('table.access')}</th>
                <th className='px-4 py-3 text-start font-semibold text-muted-foreground' scope='col'>{t('table.enrolled')}</th>
                <th className='px-4 py-3 text-end  font-semibold text-muted-foreground' scope='col'>{t('table.actions')}</th>
              </tr>
            </thead>
            <tbody>
              {data.length === 0 ? (
                ...
                <tr key={course.id} className='border-b border-border/50 hover:bg-muted/20 transition-colors'>
                  ...
                  {/* Course column: thumbnail + title + ar_title + slug */}
                  <td className='px-4 py-3'>
                    <div className='flex items-center gap-3'>
                      ...
                      <div className='min-w-0'>
                        ...
                        {course.ar_title && (
                          <p className='text-xs text-muted-foreground/70 line-clamp-1' dir='rtl'>{course.ar_title}</p>
                        )}
                      ...
                      <DropdownMenuTrigger asChild>
                        <button className='cursor-pointer'>
                          <Badge variant='outline'
                            className={cn('text-[10px] font-semibold cursor-pointer hover:opacity-80', STATE_BADGE[course.state])}>
        ...
        {meta && meta.totalPages > 1 && (
          <div className='flex items-center justify-between border-t px-4 py-3'>
            <p className='text-xs text-muted-foreground'>
              {t('table.page')} <span className='font-semibold text-foreground'>{page}</span>{' '}
              {t('table.of')} <span className='font-semibold text-foreground'>{meta.totalPages}</span>
              {' — '}<span className='font-semibold text-foreground'>{meta.total}</span> {t('table.total')}
            </p>
            <div className='flex items-center gap-1'>
              <Button variant='outline' size='sm' aria-label={t('table.prevPage')}
                className='h-8 w-8 p-0' onClick={() => onPageChange(page - 1)} disabled={page === 1}>
                <ChevronLeft className='h-3.5 w-3.5' aria-hidden='true' />
              </Button>
              <Button variant='outline' size='sm' aria-label={t('table.nextPage')}
                className='h-8 w-8 p-0' onClick={() => onPageChange(page + 1)} disabled={page === meta.totalPages}>
