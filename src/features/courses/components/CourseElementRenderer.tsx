// Copied from cyberlabs-frontend — read-only preview mode for Admin Dashboard
...
import type { TranslatedText } from '@/core/types/curriculumCourses.types';
...
const NOTE_CFG: Record<
  NoteVariant,
  { Icon: React.ElementType; border: string; bg: string; text: string; iconColor: string }
> = {
        ...
        {elements.map((el, idx) => {
          switch (el.type) {
            ...
            case 'text': {
              ...
              return (
                <p key={idx} className='mb-4 text-foreground/80 leading-7 text-[15px]'>
                  {txt}
                </p>
              );
            }
              ...
              return (
                <div key={idx} className='my-6'>
                  <div className='rounded-xl overflow-hidden border border-border shadow-md'>
                    <iframe src={el.url} className='w-full aspect-video' allowFullScreen
                      title={el.title ? tl(el.title, lang) : 'video'} />
                  </div>
                  {el.title && (
                    <p className='mt-2 text-xs text-muted-foreground text-center'>{tl(el.title, lang)}</p>
                  )}
                </div>
              );
            case 'list': {
                    ...
                    {el.items.map((item, i) => (
                      <li key={i} className='flex gap-4'>
                        ...
                        <div className='flex-1 min-w-0'>
                          ...
                          {item.image && (() => {
                            const src = item.image.imageUrl ?? (item.image.srcKey ? (imageMap[item.image.srcKey] ?? item.image.srcKey) : '');
                            return src ? (
                              <ZoomableImage src={src}
                                className={cn('mt-3 rounded-lg border border-border w-full h-auto', IMG_SIZE[item.image.size ?? 'medium'])}
                                wrapperClassName='mt-3' onZoom={openLightbox} />
                            ) : null;
                          })()}
                        </div>
                      </li>
              ...
              return (
                <div key={idx} className='my-5 rounded-xl overflow-hidden border border-border shadow-sm'>
                  <div className='flex items-center gap-2 px-4 py-2.5 bg-zinc-900 border-b border-zinc-700'>
                    ...
                    <span className='text-xs text-zinc-400'>{el.label ? tl(el.label, lang) : 'Terminal'}</span>
                  </div>
              ...
              return (
                <div key={idx} className={cn('my-5 flex gap-3 rounded-xl border p-4', cfg.border, cfg.bg)}>
                  ...
                  <div className='flex-1 text-sm leading-relaxed'>
                    {el.isLab && <strong className='font-bold me-1 text-foreground'>{lang === 'ar' ? 'اللابات:' : 'Labs:'}</strong>}
            ...
            case 'button':
              return (
                <div key={idx} className='my-4'>
                  <a href={el.href} target={el.newTab ? '_blank' : '_self'} rel='noopener noreferrer'
