...
import {
  Pencil, Trash2, Globe, EyeOff, Copy, Loader2, Clock, ChevronDown, Eye,
} from 'lucide-react';
...
export function AdminOverlayControls({ course }: { course: AdminCourse }) {
  ...
  const deleteMutation = useMutation<void, Error, void>({
    ...
    onSuccess: () => {
      ...
      invalidateList(); refetchStats(); setDeleteOpen(false);
    },
    onError: (_e: any, _v: any, ctx: any) => {
      ctx?.listSnap?.forEach(([k, v]: any)  => queryClient.setQueryData(k, v));
      ctx?.statsSnap?.forEach(([k, v]: any) => queryClient.setQueryData(k, v));
  ...
  const openPreview = (e: React.MouseEvent) => {
    ...
    if (!previewWindow.current || previewWindow.current.closed) {
      previewWindow.current = window.open(url, `preview_${course.id}`, 'width=1200,height=800');
    } else {
    ...
    <>
      <div
        className={cn(
          ...
          'translate-y-full group-hover:translate-y-0 h-full flex justify-center',
          (dropdownOpen || deleteOpen) && 'translate-y-0',
          'transition-transform duration-200 ease-out',
          'bg-zinc-900/88 backdrop-blur-md border-t border-white/10',
          'flex items-center gap-1 px-2.5 py-2',
        )}
        onClick={(e) => e.stopPropagation()}>
        <Button size='sm' className='flex-1 h-8 text-xs gap-1.5 bg-primary/90 hover:bg-primary'
        ...
        <Button size='sm' variant='outline'
          ...
          title={isAr ? 'معاينة' : 'Preview'} onClick={openPreview}>
          <Eye className='h-3.5 w-3.5' />
        </Button>
        <Button size='sm' variant='outline'
          className='h-8 w-8 p-0 border-white/20 bg-white/5 text-white/70 hover:bg-white/15 hover:text-white'
          title={isAr ? 'تكرار' : 'Duplicate'} disabled={duplicateMutation.isPending}
          onClick={(e) => { e.preventDefault(); e.stopPropagation(); duplicateMutation.mutate(undefined); }}>
        ...
        <Button size='sm' variant='outline'
          ...
          title={isAr ? 'حذف' : 'Delete'} disabled={deleteMutation.isPending}
          ...
          <DropdownMenuTrigger asChild>
            <Button size='sm' variant='outline'
              ...
              {stateMutation.isPending
                ...
                : <><CurrentIcon className='h-3 w-3' /><span className='hidden sm:inline max-w-[48px] truncate'>{currentLabel}</span><ChevronDown className='h-2.5 w-2.5 opacity-60' /></>}
            </Button>
          </DropdownMenuTrigger>
            ...
            {STATE_OPTIONS.map((opt) => {
              ...
              return (
                <DropdownMenuItem key={opt.value} disabled={isCurrent}
                  ...
                  onSelect={() => { if (!isCurrent) stateMutation.mutate(opt.value); }}>
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{isAr ? `حذف "${course.title}"؟` : `Delete "${course.title}"?`}</AlertDialogTitle>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={deleteMutation.isPending}>{isAr ? 'إلغاء' : 'Cancel'}</AlertDialogCancel>
            <AlertDialogAction
