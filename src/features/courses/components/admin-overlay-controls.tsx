...
const STATE_PILL: Record<string, string> = {
  ...
  DRAFT:
    'bg-zinc-500/15    text-zinc-300    border-zinc-500/30    hover:bg-zinc-500/25',
};
...
export function AdminOverlayControls({ course }: { course: AdminCourse }) {
  ...
  const stateMutation = useMutation({
    ...
    onMutate: async (next) => {
      ...
      const listSnap = queryClient.getQueriesData({
        queryKey: ['admin', 'courses', 'list'],
      });
      const statsSnap = queryClient.getQueriesData({
        queryKey: ['admin', 'courses', 'stats'],
      });
      ...
      queryClient.setQueriesData(
        ...
        (old: any) => {
          ...
          return {
            ...old,
            [STATE_KEY_MAP[prev]]: Math.max(
              0,
              (old[STATE_KEY_MAP[prev]] ?? 0) - 1,
            ),
      ...
      queryClient.setQueriesData(
        ...
        (old: any) =>
          old?.data
            ? {
                ...old,
                data: old.data.filter((c: AdminCourse) => c.id !== course.id),
              }
            : old,
      );
      queryClient.setQueriesData(
        ...
        (old: any) =>
          old
            ? {
                ...
                [STATE_KEY_MAP[course.state]]: Math.max(
                  0,
                  (old[STATE_KEY_MAP[course.state]] ?? 0) - 1,
                ),
              }
            : old,
      );
  ...
  const currentOpt =
    STATE_OPTIONS.find((o) => o.value === course.state) ?? STATE_OPTIONS[2];
