...
// ✅ FIXED: color values now UPPERCASE to match backend enum
import { useState } from 'react';
import { useMutation, useQueryClient } from '@tanstack/react-query';
import { adminCoursesApi } from '../services/admin-courses.api';
import type {
  AdminCourse, AdminCourseUpdateDto,
  CourseColor, CourseAccess, CourseDifficulty,
  CourseCategory, CourseContentType,
    ...
    <div className='space-y-8 max-w-3xl'>
      ...
      <section className='space-y-4'>
        ...
        <div className='grid grid-cols-1 gap-4 sm:grid-cols-2'>
          ...
          <div className='space-y-1.5'>
            ...
            <Input dir='rtl' value={form.ar_title as string}
      ...
      <section className='space-y-4'>
        ...
        <div className='grid grid-cols-1 gap-4 sm:grid-cols-2'>
          ...
          <div className='space-y-1.5'>
            <Label>Short (AR)</Label>
            <Textarea dir='rtl' rows={3} value={form.ar_description as string}
              onChange={(e) => setForm((f) => ({ ...f, ar_description: e.target.value }))} />
          </div>
          <div className='space-y-1.5'>
            <Label>Long (EN)</Label>
            <Textarea rows={5} value={form.longDescription as string}
              onChange={(e) => setForm((f) => ({ ...f, longDescription: e.target.value }))} />
          </div>
          <div className='space-y-1.5'>
            <Label>Long (AR)</Label>
            <Textarea dir='rtl' rows={5} value={form.ar_longDescription as string}
              onChange={(e) => setForm((f) => ({ ...f, ar_longDescription: e.target.value }))} />
          </div>
        </div>
      </section>
      ...
      <section className='space-y-4'>
        ...
        <div className='grid grid-cols-2 gap-4 sm:grid-cols-4'>
          <div className='space-y-1.5'>
            <Label>Access</Label>
            <Select value={form.access as string}
              onValueChange={(v) => setForm((f) => ({ ...f, access: v as CourseAccess }))}>
          ...
          <div className='space-y-1.5'>
            ...
            <Select value={form.difficulty as string}
              onValueChange={(v) => setForm((f) => ({ ...f, difficulty: v as CourseDifficulty }))}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                {(['BEGINNER','INTERMEDIATE','ADVANCED','EXPERT'] as CourseDifficulty[]).map((v) => (
                  <SelectItem key={v} value={v}>{v}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          <div className='space-y-1.5'>
            ...
            <Select value={form.contentType as string}
              onValueChange={(v) => setForm((f) => ({ ...f, contentType: v as CourseContentType }))}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                {(['PRACTICAL','THEORETICAL','MIXED'] as CourseContentType[]).map((v) => (
                  <SelectItem key={v} value={v}>{v}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          <div className='space-y-1.5'>
            ...
            <Input type='number' min={0} value={form.estimatedHours as number}
              onChange={(e) => setForm((f) => ({ ...f, estimatedHours: +e.target.value }))} />
          </div>
        </div>
        <div className='space-y-1.5'>
          <Label>Category</Label>
          <Select value={form.category as string}
      ...
      <section className='space-y-3'>
        ...
        <div className='flex gap-2'>
          ...
          {/* Show current color label */}
          <span className='flex items-center text-xs text-muted-foreground ml-2'>
            <span className={cn('h-2.5 w-2.5 rounded-full mr-1.5', COLOR_DOT[form.color as CourseColor])} />
            {form.color}
          </span>
        </div>
      </section>
      ...
      <section className='space-y-4'>
        ...
        <div className='grid grid-cols-1 gap-4 sm:grid-cols-2'>
          <div className='space-y-1.5'>
            ...
            <Input value={form.thumbnail as string}
          ...
          <div className='space-y-1.5'>
            ...
            <Input value={form.labsLink as string}
              onChange={(e) => setForm((f) => ({ ...f, labsLink: e.target.value }))}
              placeholder='https://...' />
          </div>
        </div>
      </section>
          ...
          <section key={field} className='space-y-3'>
            ...
            <div className='flex gap-2'>
              ...
              <Button variant='outline' size='icon'
                onClick={() => { addToArray(field, inputMap[field]); setterMap[field](''); }}>
