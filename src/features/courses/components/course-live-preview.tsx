// src/features/courses/components/course-live-preview.tsx
// ✅ Live preview panel — mirrors frontend CourseDetailHero + CourseCurriculum
import {
  BookOpen, Clock, Users, Star, Crown, Unlock,
  Shield, FlaskConical, Zap, Heart, Rocket,
} from 'lucide-react';
...
const COLOR_BG: Record<string, string> = {
  EMERALD: 'from-emerald-950 to-emerald-900', emerald: 'from-emerald-950 to-emerald-900',
  BLUE: 'from-blue-950 to-blue-900', blue: 'from-blue-950 to-blue-900',
  VIOLET: 'from-violet-950 to-violet-900', violet: 'from-violet-950 to-violet-900',
  ROSE: 'from-rose-950 to-rose-900', rose: 'from-rose-950 to-rose-900',
  ORANGE: 'from-orange-950 to-orange-900', orange: 'from-orange-950 to-orange-900',
  CYAN: 'from-cyan-950 to-cyan-900', cyan: 'from-cyan-950 to-cyan-900',
};
...
export function CourseLivePreview({ course, topics = [], lang = 'en' }: Props) {
  ...
  const accessBadge = ACCESS_BADGE[course.access ?? 'FREE'] ?? ACCESS_BADGE.FREE;
  ...
  return (
    <div className='rounded-2xl overflow-hidden border border-border/50 bg-background text-sm'>
      {/* ── Hero ── */}
      <div className={cn('relative bg-gradient-to-br px-6 py-8 overflow-hidden', bgGrad)}>
        {/* Matrix rain effect (simplified) */}
        <div className='pointer-events-none absolute inset-0 overflow-hidden opacity-10'
          style={{ backgroundImage: `repeating-linear-gradient(0deg, transparent, transparent 2px, ${matrixColor}22 2px, ${matrixColor}22 4px)` }} />

        {/* Breadcrumb */}
        <p className='text-xs text-white/40 mb-4'>Courses / <span className='text-white/60'>{title || 'Course Title'}</span></p>
          ...
          <div className='flex-1 min-w-0'>
            <div className='flex flex-wrap gap-1.5 mb-2'>
              <Badge variant='outline' className={cn('rounded-full text-[11px] font-bold gap-1', accessBadge)}>
                {(course.access ?? 'FREE') === 'FREE' ? <Unlock className='h-2.5 w-2.5' /> : <Crown className='h-2.5 w-2.5' />}
                {course.access ?? 'FREE'}
              </Badge>
              {course.difficulty && (
              ...
              {course.isNew && (
                <span className='inline-flex items-center gap-1 rounded-full bg-amber-500 px-2 py-0.5 text-[10px] font-bold text-white'>New</span>
              )}
              {comingSoon && (
                <span className='inline-flex items-center gap-1 rounded-full border border-white/10 bg-zinc-600/80 px-2 py-0.5 text-[10px] font-bold text-white'><Clock className='h-2.5 w-2.5' /> Coming Soon</span>
              )}
            </div>
            <h1 className='text-xl font-black text-white leading-tight'>
              {title || <span className='opacity-30 italic'>Course Title</span>}
            </h1>
            {desc && <p className='mt-1.5 text-sm text-white/60 leading-relaxed max-w-2xl'>{desc}</p>}
          </div>
        </div>
        ...
        <div className='mt-6 flex flex-wrap items-center justify-between gap-4'>
          <div className='flex flex-wrap items-center gap-x-5 gap-y-1.5'>
            {(course.totalTopics ?? 0) > 0 && (
              <div className='flex items-center gap-1.5 text-xs'>
                ...
                <span className='font-bold text-white'>{course.totalTopics}</span>
          ...
          <div className='flex items-center gap-2'>
            ...
            <button className={cn(
              ...
              comingSoon ? 'border border-white/15 bg-white/5 text-white/50 cursor-default'
                : 'bg-primary hover:bg-primary/90',
            )}>
              {comingSoon ? <><Clock className='h-3.5 w-3.5' /> Coming Soon</>
              ...
              {topics.map((topic, idx) => (
                <li key={topic.id} className='relative flex gap-4'>
                  ...
                  <div className='relative flex shrink-0 flex-col items-center'>
                    ...
                    {idx < topics.length - 1 && <div className='flex-1 w-px min-h-[16px] mt-1 bg-border/30' />}
                  </div>
                  {/* Card */}
                  <div className='mb-2 flex-1 min-w-0 rounded-xl border border-border/50 hover:border-border bg-card cursor-pointer'>
                    <button
                      ...
                      onClick={() => setOpenId((p) => (p === topic.id ? null : topic.id))}>
                      ...
                      <div className='min-w-0 flex-1'>
                        ...
                        <p className='text-sm font-semibold'>{topic.title.en || 'Untitled'}</p>
                      </div>
      ...
      {topics.length === 0 && (
        <div className='p-6 border-t border-border/40'>
          <p className='text-sm text-muted-foreground italic text-center'>Curriculum will appear here once topics are added.</p>
        </div>
      )}
    </div>
  );
