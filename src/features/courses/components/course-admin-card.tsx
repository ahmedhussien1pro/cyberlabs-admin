...
export function CourseAdminCard({ course, index = 0, view = 'grid' }: Props) {
  ...
  const diffLabel   = course.difficulty ? (isAr ? DIFF_AR[course.difficulty] : course.difficulty.charAt(0) + course.difficulty.slice(1).toLowerCase()) : null;
  const accessLabel = course.access     ? (isAr ? ACCESS_AR[course.access]   : course.access.charAt(0)     + course.access.slice(1).toLowerCase())     : null;
    ...
    return (
      <div dir={isAr ? 'rtl' : 'ltr'}
        ...
        <div className='min-w-0 flex-1'>
          ...
          {activeDesc && <p className='truncate text-xs text-muted-foreground/60 mt-0.5'>{activeDesc}</p>}
        </div>
  ...
  return (
    <motion.div
      ...
      <div className='flex flex-col flex-1 p-4 gap-2'>
        <h3 className='text-sm font-bold text-foreground leading-snug line-clamp-2'>{activeTitle}</h3>
        {activeDesc
        ...
        <div className='flex flex-wrap items-center gap-1.5 mt-auto pt-3 border-t border-border/30'>
          {diffLabel   && <Badge variant='outline' className='gap-1 text-[10px] font-semibold border-border/60 bg-muted/40'><BarChart3 className='h-3 w-3' />{diffLabel}</Badge>}
          {accessLabel && <Badge variant='outline' className={cn('gap-1 text-[10px] font-bold', ACCESS_BADGE[course.access])}><AccessIcon className='h-3 w-3' />{accessLabel}</Badge>}
          {(course.totalTopics ?? 0) > 0 && (
            <Badge variant='outline' className='gap-1 text-[10px] font-semibold text-primary border-primary/30 bg-primary/5'>
              <BookOpen className='h-3 w-3' />{course.totalTopics} {topicsLabel}
            </Badge>
          )}
          {ct && <Badge variant='outline' className='gap-1 text-[10px] text-muted-foreground border-border/40'><ct.Icon className='h-3 w-3' />{ctLabel}</Badge>}
        </div>
      </div>
    </motion.div>
  );
